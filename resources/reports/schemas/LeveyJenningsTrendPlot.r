...
qcannotation.sql = "SELECT qca.Date, qcat.Name, qcat.Color FROM qcannotation qca JOIN qcannotationtype qcat ON qcat.Id = qca.QCAnnotationTypeId";
...
if (!is.null(labkey.url.params$StartDate)) {
    ...
    qcannotation.sql=paste(qcannotation.sql, separator, "Date >= '", labkey.url.params$StartDate, "'");
    separator = " AND ";
}
if (!is.null(labkey.url.params$EndDate)) {
    sql=paste(sql, separator, "CAST(AcquiredTime AS DATE) <= '", labkey.url.params$EndDate, "'");
    qcannotation.sql=paste(qcannotation.sql, separator, "Date <= '", labkey.url.params$EndDate, "'");
}

sql =  paste(sql, " ORDER BY Sequence, AcquiredTime DESC");

# call the selectRows function to get the data from the server
labkey.data <- labkey.executeSql(baseUrl=labkey.url.base, folderPath=labkey.url.path, schemaName="targetedms", sql=sql, colNameOpt="rname");
